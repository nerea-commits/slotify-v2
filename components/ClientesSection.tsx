...
export default function ClientesSection({ empresaId }: ClientesSectionProps) {
  ...
  async function guardarCliente() {
    ...
    if (editando && vistaDetalle) {
      ...
      const actualizado = { ...vistaDetalle, ...form };
      setVistaDetalle(actualizado as Cliente);
    } else {
      const { error } = await supabase
        .from('clientes')
        .insert({
    ...
    return (
      <div style={{ minHeight: '100vh', background: C.bg, color: C.text }}>
        ...
        <div style={{ padding: 20, maxWidth: 600, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {vistaDetalle.telefono ? (
                <a href={`tel:${vistaDetalle.telefono}`}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none', color: C.text }}>
              ...
              {vistaDetalle.correo_electronico ? (
                <a href={`mailto:${vistaDetalle.correo_electronico}`}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none', color: C.text }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: C.greenBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {/* Historial de citas */}
          <div style={{ background: C.surface, borderRadius: 16, padding: 20, marginBottom: 16 }}>
            <h3 style={{ fontSize: 13, fontWeight: 600, color: C.textSec, marginBottom: 16, textTransform: 'uppercase', letterSpacing: 0.8 }}>
              Historial de citas
            </h3>
            {loadingCitas ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {historialCitas.map(cita => (
                  <div key={cita.id} style={{
                    ...
                    borderLeft: `3px solid ${estadoColor(cita.estado)}`,
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  }}>
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 600 }}>
                        {cita.servicios?.nombre || cita.servicio_nombre_libre || 'Servicio'}
                      </p>
                      <p style={{ fontSize: 12, color: C.textSec }}>
                        {formatFecha(cita.hora_inicio)} · {cita.hora_inicio?.substring(11, 16)}
                      </p>
                    </div>
                    <span style={{ fontSize: 11, color: estadoColor(cita.estado), fontWeight: 600, textTransform: 'capitalize' }}>
                      {cita.estado}
                    </span>
                  </div>
                ))}
            ...
            <div style={{ background: C.redBg, borderRadius: 12, padding: 16, border: `1px solid ${C.red}` }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setConfirmDelete(false)}
                  style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', background: C.surfaceAlt, color: C.text, cursor: 'pointer', fontSize: 13 }}>
                  Cancelar
                </button>
                <button onClick={eliminarCliente}
                  style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', background: C.red, color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                  Sí, eliminar
                </button>
              </div>
            </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {busqueda && (
            <button onClick={() => setBusqueda('')}
              style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: C.textSec }}>
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {clientesFiltrados.map(cliente => (
                ...
                >
                  {/* Avatar */}
                  <div style={{
                    width: 44, height: 44, borderRadius: 12, background: C.greenBg,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 18, fontWeight: 700, color: C.green, flexShrink: 0,
                  }}>
  ...
  function ModalCliente() {
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
        ...
        <div style={{ background: C.surface, borderRadius: '20px 20px 0 0', padding: 24, width: '100%', maxWidth: 500, paddingBottom: 32 }}
          onClick={e => e.stopPropagation()}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <h3 style={{ fontSize: 18, fontWeight: 700 }}>{editando ? 'Editar cliente' : 'Nuevo cliente'}</h3>
            <button onClick={() => setModalAbierto(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.textSec }}>
              <X className="w-5 h-5" />
            </button>
          </div>
