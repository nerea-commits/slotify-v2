...
export default function NuevaCitaModal({
  ...
  useEffect(() => {
    if (open && empresaId) {
      ...
      if (preselectedTime) {
        ...
        setHoraFin(`${String(Math.floor(totalMin / 60) % 24).padStart(2, '0')}:${String(totalMin % 60).padStart(2, '0')}`);
      } else {
  ...
  async function cargarEstados() {
    const { data } = await supabase.from('estados_cita').select('*').eq('empresa_id', empresaId).eq('activo', true).order('orden');
    setEstadosCita(data || []);
  ...
  useEffect(() => {
    ...
    if (digits.length >= 6) {
      debounceRef.current = setTimeout(() => buscarClientePorTelefono(digits), 400);
    } else {
    ...
    try {
      ...
      if (clientes && clientes.length > 0) {
        ...
        if (found) {
          ...
          // Fetch citas WITH estado_detallado for accurate risk calculation
  ...
  async function validarSolapamiento(fecha: string, inicio: string, fin: string): Promise<string | null> {
    ...
    const { data } = await supabase
      .from('citas').select('id, hora_inicio, hora_fin, estado')
      .eq('empresa_id', empresaId).eq('profesional_id', profesionalId)
      .eq('blocks_time', true)
      .lt('hora_inicio', hFinISO).gt('hora_fin', hInicioISO);
    if (data && data.length > 0) {
      const horaConflicto = data[0].hora_inicio?.substring(11, 16) || '';
  ...
  async function guardar() {
    ...
    const citaData: any = {
      empresa_id: empresaId, profesional_id: profesionalId, cliente_id: clienteId,
      hora_inicio: hInicio, hora_fin: hFin,
      estado: estadoFinal, blocks_time: blocksTime(estadoFinal),
      notas: notas.trim() || null,
    };

    const svcObj = servicios.find(s => s.id === servicio);
    if (svcObj) citaData.servicio_id = svcObj.id;
    else if (servicio.trim()) citaData.servicio_nombre_libre = servicio.trim();
  ...
  // ═══ Build concrete alert message from fiabilidad ═══
  const alertInfo = fiabilidad && fiabilidad.alertLevel !== 'none' && fiabilidad.alertMessage ? {
    ...
    detail: (() => {
      const parts: string[] = [];
      if (fiabilidad.noShowsReales > 0) parts.push(`${fiabilidad.noShowsReales} no-show${fiabilidad.noShowsReales !== 1 ? 's' : ''} sin justificar`);
      if (fiabilidad.noShowsJustificados > 0) parts.push(`${fiabilidad.noShowsJustificados} justificado${fiabilidad.noShowsJustificados !== 1 ? 's' : ''}`);
      if (fiabilidad.cancelacionesTardias > 0) parts.push(`${fiabilidad.cancelacionesTardias} cancelación${fiabilidad.cancelacionesTardias !== 1 ? 'es' : ''} tardía${fiabilidad.cancelacionesTardias !== 1 ? 's' : ''}`);
      if (parts.length === 0) return `${fiabilidad.completadas} completadas de ${fiabilidad.totalCitas}`;
      return parts.join(' · ') + ` · ${fiabilidad.completadas} completadas de ${fiabilidad.totalCitas}`;
    })(),
    badge: fiabilidad.displayLabel || fiabilidad.label,
    badgeColor: fiabilidad.riskColor || fiabilidad.color,
  } : null;
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#111827', borderRadius: '20px 20px 0 0', padding: 24, width: '100%', maxWidth: 500, maxHeight: '92vh', overflow: 'auto', paddingBottom: 32 }} onClick={e => e.stopPropagation()}>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: '#F1F5F9' }}>Nueva cita</h3>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8' }}><X className="w-5 h-5" /></button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* ═══ ALERTA FIABILIDAD — datos concretos ═══ */}
          {alertInfo && (
            <div style={{
              padding: '10px 14px', borderRadius: 10, display: 'flex', alignItems: 'flex-start', gap: 10,
              background: alertInfo.bgColor, border: `1px solid ${alertInfo.borderColor}`,
            }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                  <p style={{ fontSize: 12, fontWeight: 600, lineHeight: 1.4, color: alertInfo.color }}>
                    {alertInfo.message}
                  </p>
                  <span style={{
                    fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 4,
                    background: alertInfo.badgeColor + '18', color: alertInfo.badgeColor,
                    border: `1px solid ${alertInfo.badgeColor}33`,
                  }}>
                    {alertInfo.badge}
                  </span>
                </div>
          ...
          <div>
            ...
            {servicios.length > 0 ? (
              <select value={servicio} onChange={e => setServicio(e.target.value)}
                style={{ width: '100%', padding: '12px 14px', background: '#1A2332', border: '1px solid rgba(148,163,184,0.06)', borderRadius: 12, color: servicio ? '#F1F5F9' : '#4B5563', fontSize: 15, outline: 'none', boxSizing: 'border-box', appearance: 'none' as const }}>
